 ...
 * 2.程序中载入$this->load->library('buyfunds_mem',[配置键名]);
 * 3.程序中调用$this->buyfunds_mem->set('缓存键名','缓存内容','过期时间',[是否压缩]);
 * 4.程序中调用$this->buyfunds_mem->get('缓存键名');
 *---------------------------------------------------------------
...
class Buyfunds_mem
{
	...
	*/
	...
	(
		'server' => array(
			array('host'=>'192.168.15.104','port'=>11211)
		),
		'prefix'   => '',
		'expire'   => 5,
   ...
   /**
    ...
    public static function getInstance()
	{
        if($this->instance == null)
            $this->instance = new buyfunds_mem('memcache');
	...
	*/
	...
	private function connect()
	{
		...
		foreach($this->options['server'] as $rs)
		{
			...
			$this->handle->addServer($rs['host'], $rs['port']);
            $flag = $this->handle->setSaslAuthData($rs['muser'], $rs['mpassword']);

			if(FALSE === $flag)
				$this->logWrite('buyfunds_mem connect fail|host:'.$rs['host']);
		}
	}
	...
	{
		$flag = $this->handle->set($this->options['prefix'] . $keys, $value, $expire);

		if(FALSE === $flag)
			$this->logWrite('buyfunds_mem set fail|keys:'.$keys);
	...
	public function delete($keys)
	{
		...
		if(FALSE === $flag)
			$this->logWrite('buyfunds_mem delete fail|keys:'.$keys);
	...
	{
		!defined('APPPATH') && exit('buyfunds_mem:`APPPATH` constants undefined');
...
/* End of file buyfunds_mem.php */
/* Location: ./application/libraries/buyfunds_mem.php */