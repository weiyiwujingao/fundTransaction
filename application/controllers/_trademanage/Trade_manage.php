...
class Trade_manage extends MY_Controller 
{
    ...
    **/
    ...
    { 	
		...
		if($this->_data['userInfo']['Mobile'])exit('非法路径');
    ...
    public function ajaxRegAction(){
		...
		if($this->_data['userInfo']['Mobile']){
			echo json_encode(array('flag'=>'10001', 'msg'=>'非法路径')); exit;
		}
	...
	public function bindphponesuccess()
	{
		...
		$this->load->view('_trademanage/bindphponesuccess.html',$this->_data);
	}
    ...
    **/
    ...
    public function modMobileCode(){
        ...
        $tid = $this->input->post('tid', true);
        logs('获取修改手机号验证码  |-$type-|'.$type.'   |-$tid-|'.$tid,$logFile);
        //参数是否完整
        if(!$type){
			echo json_encode(array('flag'=>'10001', 'msg'=>'参数错误，请重试')) ; exit;
        }
	...
	public function verifyCode(){
        ...
        logs('|-验证修改手机验证码 verifyCode-|' . '|-$userID-|'.$userID.'|-$ip-|'.$ip.'|-$dcode-|'.$dcode
            .'|-$tid-|'.$tid.'|-$stid-|'.$stid.'|-$mkid-|'.$mkid, $logFile);
