	...
	//setTimeout(function(){
		$('.fundInp').Fund({
				...
				$.ajax({
					...
					success: function(d){
						...
						for(var i = 0; i<d['info'].length; i++){
							html = html + '<tr datacode="'+d['info'][i]['code']+'"><td><span><span>'+d['info'][i]['code']+'</td><td>'+d['info'][i]['code']+'</td><td>'+d['info'][i]['types']+'</td><td>'+d['info'][i]['sname']+'</td></tr>';
						}
			...
			click:function(o){//dd内a标签点事件 表格tr的点击事件
				...
				$(o).parents('.fund_suggest').css('display','none');//alert('main');
